...
- **Account-delete UX: password-confirm vs type-to-confirm drift.** The smoke brief and historical spec discussion describe a "type your username to confirm" flow for the irreversible delete. Current implementation on `feat/s2-account-gdpr` uses a password-confirm dialog (`apps/web/src/components/account/DeleteAccountDialog.tsx` or equivalent — triggered from `apps/web/src/app/settings/account/page.tsx`). Both verify identity; the password route is arguably stronger. Decide which is canonical and reconcile the other artifact (spec or UI).
  - **Fix shape**: if password-confirm is canonical, update `docs/specs/s2-account-gdpr.md` §UX. If type-to-confirm is canonical, swap the dialog body for a text input that must equal the viewer's `username`. No backend change either way — the backend already re-authenticates via `auth.api.getSession` + password check.
  - **Raised**: 2026-04-19 during Wave A Gate 4.

- **Export endpoint verb/path docs drift.** `apps/backend/src/routes/account.ts:126` registers the data export as `POST /users/me/export` (returns 200 with the JSON body inline, no `Content-Disposition`). Earlier smoke notes and a handful of internal references assumed `GET /api/v1/account/export`. The implementation is correct (POST is defensible — the export is non-idempotent because it writes an audit log row); docs + any curl snippets need to match.
  - **Fix shape**: grep `/api/v1/account/export` and `/users/me/export` across `docs/` and update to `POST /api/v1/users/me/export`. The frontend call site [apps/web/src/lib/account-api.ts:8](../apps/web/src/lib/account-api.ts#L8) is already correct.
  - **Raised**: 2026-04-19 during Wave A Gate 4 (first curl attempt returned 404).

- **Export filename pattern drift.** Backend produces `user-data-export-${username}-${Date.now()}.json` (`apps/backend/src/routes/account.ts:361`). Earlier gate writeups and the client fallback in `apps/web/src/lib/account-api.ts` both reference `ai-herders-export-*.json`. Low impact — the client reads `Content-Disposition` when CORS exposes it (already queued under "Export download uses client-generated filename" in the S2→S3 section), and the fallback is cosmetic. Pick one convention and align both.
  - **Raised**: 2026-04-19 during Wave A Gate 4.
