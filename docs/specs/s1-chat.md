...
- [ ] **R2 (REQ-030, REQ-037)**: Each successful insert advances `message_seq.seq` by 1 for that `roomId`; the new `message.seq` equals the updated `message_seq.seq`; within a room, observed `seq` values are strictly increasing. Test-name literal MUST contain **both** IDs so `pnpm trace` resolves each: e.g. `describe("REQ-030/REQ-037 seq advances by 1 and is strictly increasing", …)`. Single-ID names will fail the trace.
