...
- [ ] **R12 (REQ-041)**: Socket connect emits `presence.state` with `{ type: "presence.state", userId, state: "online", since }` via `io.emit` (global fanout — see §8 Q2 for the S2 scoping follow-up); socket disconnect emits `state: "offline"` the same way. `since` MUST be the ISO timestamp (e.g. `new Date().toISOString()`) of the connect/disconnect event — the field is mandatory per `PresenceStateEvent` in [protocol.ts:55-60](../../packages/shared/src/protocol.ts#L55-L60). S1 is binary; AFK is S2. Asserted by a second socket observing the state events for the first socket's lifecycle.
