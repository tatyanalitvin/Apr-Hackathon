...
- As Alice, within `general` the `seq` values I observe are strictly increasing with no gaps or duplicates, even when carol and bob send concurrently. (REQ-032)
...
- [ ] **R1 (REQ-029, REQ-030)**: `POST /api/v1/rooms/:id/messages` with a valid body (zod `sendMessageSchema`) from an authenticated member of the room inserts one `message` row, returns 201 with the `MessagePayload` shape from `protocol.ts`, and the row's `body` equals the NFC-normalized, control-char-stripped input. Body over 3072 bytes → 400 (zod) — REQ-030 body-size guard (S1 uses a 3072-byte cap; v4 catalog allows up to 4096, flagged as an intentional tighter cap for the hackathon demo). Empty body → 400. Non-member / unauth → 403 / 401 respectively.
- [ ] **R2 (REQ-032)**: Each successful insert advances `message_seq.seq` by 1 for that `roomId`; the new `message.seq` equals the updated `message_seq.seq`; within a room, observed `seq` values are strictly increasing. Test-name literal must contain the ID so `pnpm trace` resolves it: e.g. `describe("REQ-032 seq advances by 1 and is strictly increasing", …)`. (v4 REQ-032 = Server-assigned ordering; body size lives on R1 and offline delivery is out of S1 scope — see ADR-0006 for the old pre-v4 mapping.)
...
- [ ] **R8 (REQ-034)**: After `POST …/messages` succeeds, a `buildApp()` teardown + re-bootstrap still returns the message via `GET …/messages`. (Implicit in Testcontainers reuse: the Postgres container survives backend restart within the test file.)
...
- [ ] **R10 (REQ-039)**: Socket.IO heartbeat is left at Socket.IO's defaults (pingInterval 25s, pingTimeout 20s — no override needed). `connectionStateRecovery` is already configured in [socket.ts:19-22](../../apps/backend/src/socket.ts#L19-L22); spec-level ack is "default heartbeat present + `connectionStateRecovery` enabled". Asserted by reading `io.engine.pingInterval` and `io.engine.pingTimeout` (stable public API) in a smoke test, not by timing a disconnect. Do NOT assert against `io.engine.opts` — that's an internal shape that has shifted across Socket.IO releases.
- [ ] **R11 (REQ-040)**: `socket.emit("room.subscribe", roomId, ack)` causes the server to (a) verify the caller is a member of the room (respond `ack({ ok: false, roomHeadSeq: "0" })` if not — no join happens; the frozen ack type has no `error` field, see §8 Q3), (b) join the underlying Socket.IO room, (c) respond `ack({ ok: true, roomHeadSeq })` with the current `message_seq.seq` for the room as a string. `room.unsubscribe` leaves the Socket.IO room (no ack needed — matches `protocol.ts`). **Frontend note**: branch on `ack.ok`, NEVER on `roomHeadSeq === "0"` — an empty freshly-seeded room legitimately has `roomHeadSeq === "0"` for any member before the first message.
- [ ] **R12 (REQ-041)**: Socket connect emits `presence.state` with `{ type: "presence.state", userId, state: "online", since }` via `io.emit` (global fanout — see §8 Q2 for the S2 scoping follow-up); socket disconnect emits `state: "offline"` the same way. `since` MUST be the ISO timestamp (e.g. `new Date().toISOString()`) of the connect/disconnect event — the field is mandatory per `PresenceStateEvent` in [protocol.ts:55-60](../../packages/shared/src/protocol.ts#L55-L60). S1 is binary; AFK is S2. Asserted by a **two-distinct-user** setup: observer socket (user A) watches subject socket (user B) connect → online; B disconnects → offline. **Known S1 limitation (documented in §7, test constraint here)**: S1 emits on every connect/disconnect, so two tabs for the same user will flap (connect-A online, connect-B online, close-B offline while A still live). Tests MUST use distinct users. If someone writes a same-user multi-tab test against this spec, they're testing S2 behavior.
- [ ] **R13**: `pnpm db:seed` (or `pnpm --filter backend db:seed`) creates `alice`, `bob`, `carol` (via `auth.api.signUpEmail` so password hashes match production), one `room` named `general` (visibility public, kind group) with all three as members, and 3 seed messages authored across the three users. Running the script twice results in no duplicate rows (idempotent: use `ON CONFLICT DO NOTHING` or pre-check by username / room name). (Seed is demo infra, not a v4 REQ. v4 REQ-049 = Stage 1 exit criteria, which is §9 gate criteria — not this bullet. See ADR-0006.)
...
| S→C | `message.new` | REQ-034 | Emitted from the message send handler via `request.server.io.to(roomId).emit("message.new", evt)` (io plumbing — see below). `evt.seq === evt.roomHeadSeq === evt.message.seq`. |
...
**io plumbing (how `messages.ts` reaches the io instance)**: use a Fastify decorator. In `server.ts`, immediately after `const io = await createSocketIO(app.server)`, call `app.decorate("io", io)`. The route handler then reaches it via `request.server.io.to(roomId).emit(...)`. TypeScript augmentation: add `declare module "fastify" { interface FastifyInstance { io: ChatIOServer } }` in a `src/types/fastify.d.ts` (or inline at the top of `server.ts`). Rationale: keeps `createSocketIO`'s lifecycle control in `server.ts`, avoids an import cycle between `messages.ts` and `socket.ts`, and reuses a Fastify-native pattern. Do NOT export a module-level `io` singleton from `socket.ts` — that would split ownership of the instance and make test-harness teardown fragile (the factory is the source of truth for the instance).
...
9. [ ] **Heartbeat + presence lifecycle (R10, R12 / REQ-039, REQ-041)** — Assert Socket.IO defaults (`pingInterval 25s`, `pingTimeout 20s`) via `io.engine.pingInterval` / `io.engine.pingTimeout` (stable public API per R10 — do NOT use `io.engine.opts`). Emit `presence.state` on connect/disconnect with `since: new Date().toISOString()`. Broadcast scope: `io.emit` globally in S1 (simple; §8 open question flags whether to scope to rooms the user is in). Test: two sockets; socket A observes socket B's online→offline transition.
...
11. [ ] **Seed script (R13 / REQ-049)** — `scripts/seed.ts` + `pnpm db:seed` in `apps/backend/package.json`. Idempotency test: run it twice via `pnpm db:seed`, assert user/room/message counts match between runs. **Dependency**: step 1 calls `auth.api.signUpEmail`, which is wired on `feat/s1-auth`. If this seed task lands on `main` before `feat/s1-auth` merges, `pnpm db:seed` is red. Either (a) gate this task behind the auth merge, or (b) land it on a branch rebased on top of `feat/s1-auth`. Do NOT work around it with raw SQL inserts — password hashes would diverge from production.
...
- **CSR auth bypass window (S3, security)** — [socket.ts:19-22](../../apps/backend/src/socket.ts#L19-L22) sets `connectionStateRecovery.skipMiddlewares: true`. Consequence: a socket whose better-auth session was revoked mid-disconnect will "recover" the old authenticated state for up to 2 minutes (the `maxDisconnectionDuration`). S1 accepts this — the alternative (running the auth middleware on recovery) costs a DB round-trip per reconnect and the recovery window is short. S3 hardening task: either (a) flip `skipMiddlewares: false` and re-auth on recovery, or (b) on `revokeSession`, publish to a Redis channel that each backend subscribes to and force-disconnects any live socket for that session. (a) is cheaper; (b) is tighter.
...
- **Same-user multi-tab presence flap (S2)** — S1 emits `online`/`offline` on every socket connect/disconnect. A user with two tabs open will emit `online` twice on open and an incorrect `offline` when either tab closes while the other is still live. S1 ships the flap; R12's test setup avoids it by using distinct users. Fix when presence UX matters (S2): Redis-backed per-user connection counter — `INCR presence:{userId}` on connect, `DECR` on disconnect; emit `online` on the 0→1 transition, `offline` on 1→0, no event on any other transition. Cost: one Redis round-trip per connect/disconnect. Not worth it for the S1 demo (one tab per user), but test writers need to know the constraint — hence R12's explicit "distinct users" note.
...
- [ ] **`clientMessageId` — where does it live on the wire, and where is dedup enforced?** Neither `sendMessageSchema` (`packages/shared/src/dto.ts`) nor the `message` table (`packages/shared/src/schema.ts`) has a `clientMessageId` field, but REQ-033 explicitly requires idempotent re-submits. Options:
    ...
    - **(c)** Add only to DTO (client-side generated UUID in the POST body) + Redis dedup (no DB column). Schema unchanged, dto extended. **This still requires human approval** — `dto.ts` is frozen per CLAUDE.md non-negotiable #5 and CHAT_AGENT_BRIEF.md hard-constraint #7. Any field added to `sendMessageSchema` must be signed off, symmetric with (a). Non-durable by design: dedup is lost on Redis flush and past the 5-minute TTL, so a user who double-submits after a ≥5-min pause (or after a Redis restart) WILL get duplicate rows. Acceptable for the 24h demo; frontend agent must NOT design UI assuming durable dedup.
    - **Recommendation**: (c). Still needs approval (dto change). The realistic win-case — double-click or retry within seconds — IS protected; the holes (Redis flush, ≥5-min pause) are acceptable for the 24h demo. Schema stays untouched.
...
| --- | --- | --- |
| REQ-029 | POST happy path + unauth 401 + non-member 403 | integration |
| REQ-030 | oversized body (>3072 bytes) → 400 (zod) | integration |
| REQ-031 | NFC + control-char strip (helper unit + end-to-end) | unit + integration |
| REQ-032 | 100 parallel sends → unique contiguous seqs; strictly increasing ordering | integration (concurrency) |
| REQ-033 | same clientMessageId → same message.id + same seq | integration (blocked on §8) |
| REQ-034 | messages survive `buildApp()` re-bootstrap (persistence) | integration |
| REQ-035 | history fromSeq/toSeq slicing + limit cap + bigint-as-string | integration |
| REQ-036 | second socket receives `message.new` with {seq, roomHeadSeq} within same tick | integration (Socket.IO client) |
