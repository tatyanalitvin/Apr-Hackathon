...
**Scope**: REQ-075 … REQ-085 (supported types, upload methods, metadata, access control, access revocation, post-revocation persistence, 20 MB/3 MB caps, local-FS storage, DM parity). Depends on `s2-dms.md` for the freeze predicate that gates DM attachment sends.
...
- [ ] **R4 (REQ-077 original filename + comment)**: Upload persists `originalName` verbatim from the multipart part's `filename` ([schema.ts:328](../../packages/shared/src/schema.ts#L328)); NFC-normalized (same pipeline as message body per REQ-031). Optional `comment` stored in `attachment.comment` ([schema.ts:334](../../packages/shared/src/schema.ts#L334)). Download (R11) serves with `Content-Disposition: attachment; filename*=UTF-8''<percent-encoded>` to survive unicode (`RFC 5987`). Test: upload a file named `"файл.txt"`, assert stored `originalName === "файл.txt"`, assert download's Content-Disposition parses to that exact filename.
...
- [ ] **R10 (REQ-083 no path traversal)**: `attachmentId` is a generated `crypto.randomUUID()` (not user-derived). `originalName` is NEVER used in the on-disk path — only in the DB row (R4) and in the download's `Content-Disposition`. Test: upload with `originalName="../../../etc/passwd"`, assert the file is written under `UPLOAD_DIR/<yyyy>/<mm>/<uuid><ext>` where `<ext>` is sanitized to `[a-zA-Z0-9]{0,8}` (longer or non-alphanumeric → empty). Assert no file was created outside `UPLOAD_DIR`.
...
- [ ] **R12 (REQ-075, REQ-076 link step on message send)**: `POST /api/v1/rooms/:id/messages` with `attachmentIds: [a, b, c]` MUST, inside the same transaction as the `message` INSERT + seq allocation: (a) SELECT the 3 rows `WHERE id IN (a,b,c) AND uploader_id = :caller AND room_id = :targetRoomId AND message_id IS NULL FOR UPDATE`, (b) UPDATE SET `message_id = :newMsgId` for all 3. If any row is missing from the SELECT → 400 `attachment_invalid` with NO message inserted (transaction rolls back, seq NOT advanced). Tests: four branches — (a) happy path, 3 attachments link, (b) attachment owned by bob → 400, (c) attachment uploaded to a different room → 400, (d) attachment already linked (messageId not null) → 400.
- [ ] **R13 (REQ-084 DM parity with freeze check)**: Sending an attachment-bearing message to a DM room uses the same handler; the `s2-dms.md` R5 freeze predicate runs BEFORE R12's link step. If frozen → 409 `dialog_frozen` with NO message, NO seq consumed, and the attachment row stays `messageId=NULL` (orphan, GC'd in S3). Upload itself (R1) does NOT check freeze — freeze gates writes to the conversation, not the scratch-pad upload. Test: freeze an alice-bob DM; alice uploads → 201; alice sends-with-attachment → 409; assert attachment row still has `messageId IS NULL`.
- [ ] **R14 (REQ-080 persistence after uploader loses access)**: Alice uploads, sends (attachment linked to a message), then alice is kicked from the room (REQ-094, S2 room-management owns the endpoint; test uses direct DB delete of `room_member` row as a stand-in — contract item cited). Bob (still a member) can still GET the attachment. Alice can NOT (R6). File on disk still exists. Test: after kick, assert `fs.existsSync(UPLOAD_DIR + storagePath)` is true AND bob's GET returns 200 AND alice's GET returns 403.
...
8. [ ] **Link step in send (R12)** — extend `apps/backend/src/routes/messages.ts`. Test file: `messages-send-with-attachments.test.ts`. Four branches: (a) happy 3-attachment link, (b) wrong uploader → 400, (c) wrong room → 400, (d) already-linked → 400. Each asserts transaction rollback (no message row, no seq advance).
