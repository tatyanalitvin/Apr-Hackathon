...
- [ ] **R1 (REQ-050)**: `GET /api/v1/friends` returns `{ friends: Array<{ userId, username, name, friendedAt }> }` for the authenticated user — one entry per row in `friendship` where the caller is `userAId` OR `userBId`. Ordered by `friendedAt` DESC. 401 without session. **Shape verified against schema**: `user` has both `name text notNull` ([schema.ts:47](../../packages/shared/src/schema.ts#L47)) and `username text notNull unique` ([schema.ts:55](../../packages/shared/src/schema.ts#L55)) — display name vs handle are distinct, both safe to select.
...
- [ ] **R4 (REQ-053)**: If `user_block` has a row with `byId=target, targetId=caller` (i.e. target has blocked caller), the endpoint returns **the exact same HTTP shape as R2's real-insert path**: `201 { id: <fabricated UUID v4>, status: 'pending' }`. No `friend_request` row is created; the UUID is generated per call and NOT persisted anywhere (acceptable — the only consumer is the caller, who treats it as opaque). Test asserts: (a) HTTP 201, (b) response shape byte-identical to R2's shape at the key+type level (no `id: null`, no `status: 'sent'`), (c) zero rows in `friend_request` for the `(fromId, toId)` pair. Rationale: REQ-053 explicitly "avoids leaking the ban"; distinct response shapes defeat that. **Known residual leak (documented, not fixed here)**: if the caller subsequently GETs `…/requests?direction=outgoing`, the sentinel-success row won't appear (there's no DB row). REQ-053's "requester UI shows 'Request sent'" is satisfied by the caller's client-local optimistic state after the 201 — if the UI re-fetches outgoing and cross-references, it CAN detect the block. Closing this would require materializing a shadow row visible only to the sender; complexity not worth it at S2 scale. See §7 follow-ups.
...
- [ ] **R10 (REQ-057 block)**: `POST /api/v1/friends/requests/:id/block` — caller must be `toId`. In a single transaction: UPDATE `friend_request` SET `status='rejected', respondedAt=now()` on the specific `:id` row; **also** UPDATE any other pending `friend_request` rows in either direction between the pair to `status='rejected', respondedAt=now()` (mirrors R16's reject-all-pending clause — otherwise an outgoing pending from `toId` to `fromId` would survive the block, leaving an asymmetric state); INSERT INTO `user_block (byId=caller, targetId=fromId)` ON CONFLICT DO NOTHING; DELETE any `friendship` row between the pair (there shouldn't be one since the request is pending, but defensive). 200 `{ status: 'blocked' }`. No Socket.IO event.
...
- [ ] **R20 (transverse)**: No endpoint returns data about a user who has blocked the caller. `GET /api/v1/friends` on Alice omits bob if bob blocked Alice (even though `friendship` is symmetric, the block deletes the row per R16 — so this is covered mechanically). The stricter rule is: the friend-request listing MUST NOT reveal "bob blocked you" via response shape. Test: confirm sentinel-success (R4) response body is structurally identical to a real insert — same keys, same value types, `id` is a valid UUID in both. Timing parity is NOT asserted (CI variance makes ±50ms flaky and a shape match is what matters). Known residual leak via `GET …?direction=outgoing` is documented in R4 and §7.
...
**Critical ordering**: the rate-limit middleware MUST run BEFORE the R4 block check. If the block check fires first, a blocked requester's attempts are "free" (no DB row → no bucket increment is tempting to implement) and the 20/24h cap can be bypassed by enumeration. Correct order on `POST /friends/requests`:

1. Auth (R19)
2. zod parse (R2)
3. Rate-limit increment + check (R5) — burns the bucket even on block-path attempts
4. Username resolution → userId (R2)
5. Self-check (R2 — 400 if `fromId === toId`)
6. Block check (R4) — sentinel-success if target blocked caller
7. Duplicate check (R6 — UPDATE pending, 409 already_friends, 409 request_declined)
8. INSERT (R2)

Tests MUST assert that step 3 happens before step 6: fire 21 POSTs from a blocked caller; the 21st returns 429 (not another sentinel 201).
...
2. [ ] **DTO additions (15a — was task 15, front-loaded)** — BLOCKED on §8 Q1 + Q3 approval. Then extend `sendFriendRequestSchema` per Q1 (union of `toUsername` / `toUserId`). Task 3 depends on this landing. Schema.ts migration per Q2/Q4 (if approved) lands here too. **Do NOT proceed to task 3 until DTO is in.**
3. [ ] **`POST /api/v1/friends/requests` happy path (R2 / REQ-051)** — `friends-send.test.ts`. Tests: valid insert (both `toUsername` and `toUserId` branches), 404 on unknown username/userId, 400 on self, dto validation edges.
4. [ ] **`GET /api/v1/friends` (R1 / REQ-050)** — `friends-list.test.ts`. Fixture: alice + bob friendship; assert bob appears for alice and alice appears for bob. Ordering assertion: friendedAt DESC with two rows at different timestamps.
5. [ ] **Sentinel-success under block (R4 / REQ-053)** — `friends-send-blocked.test.ts`. Insert `user_block(by=bob, target=alice)`; alice POSTs request to bob; assert 201 + **shape-identical** to real insert (same keys, `id` is a valid UUID, `status='pending'`) + zero rows in friend_request. No timing assertion (see R20).
6. [ ] **Duplicate-request branches (R6 / REQ-055) + Q5 decision path** — `friends-send-duplicate.test.ts`. Three branches: pending→UPDATE (200 + same id), accepted→409 `already_friends`, rejected→**409 `request_declined`** per Q5 recommendation (a). If human flips Q5 to (b), swap the rejected-branch test to assert UPDATE-back-to-pending; one-branch change.
...
14. [ ] **Remove friend (R12 / REQ-059)** — `friends-remove.test.ts`. Either party DELETEs; row gone; idempotent 204.
15. [ ] **User-to-user block (R16 / REQ-073)** — `users-block.test.ts`. POST endpoint; transaction deletes friendship, rejects pending requests in both directions, inserts user_block. Self-block → 400. Idempotent.
16. [ ] **Unblock (R18 / REQ-074)** — `users-unban.test.ts`. DELETE endpoint removes user_block only; friendship is NOT restored; idempotent 204.
17. [ ] **Gate dry-run** — Manual: alice sends request to bob via REST; bob (second browser) sees it in contacts listing; bob accepts; alice's tab receives Socket event; alice removes bob; alice blocks bob; alice unblocks bob; re-friend still requires a fresh request (REQ-074 explicit).

## 7. Out of scope / follow-ups

- **REQ-056 nightly sweep** — S3 (REQ-163). This spec ships the read-side filter only.
- **REQ-053 outgoing-listing enumeration residual** — sentinel-success rows are not in the DB, so `GET /friends/requests?direction=outgoing` won't show them and a client that cross-references "I hit 201 but my outgoing list doesn't show the row" CAN detect the block. Closing this requires materializing a shadow row visible only to the sender, which adds two filter branches to every outgoing-list query and a GC path for shadows when the block is released. Not worth it at S2 scale; revisit in S3 if the threat model elevates.
...
- [ ] **Q4 — Normalization of `friendship.userAId < userBId` — enforced where?** Schema comment says "normalize so userAId < userBId" but there's no CHECK constraint. Options:
    ...
    - **Recommendation**: (a). One line in the migration (`CHECK (user_a_id < user_b_id)`); the comment-only convention is exactly the thing that breaks when a future agent writes an `INSERT INTO friendship` in a second place (e.g. an S3 migration-driven data repair script). The "hackathon pace" argument doesn't hold — the CHECK is cheaper than a grep-based CI rule.

- [ ] **Q5 — Re-open of a previously rejected friend request (R6 branch).** REQ-055 authorizes UPDATE on a *pending* duplicate; it is silent on rejected. Options:
    - **(a)** Rejected → 409 `request_declined`. Terminal per `(fromId, toId)` pair until target clears (no mechanism in scope). Prevents A re-prompting B through the decline button.
    - **(b)** Rejected → UPDATE status back to 'pending' + refresh `message` + clear `respondedAt`. Treats decline as soft; enables harassment within R5's 20/24h cap (still 20 pokes/day).
    - **(c)** Rejected → 409, but add an explicit "unfriend-request" API for target to clear the decline record. Doubles surface; not in scope.
    - **Recommendation**: (a). Aligns with the spirit of REQ-057 (decline is a response, not a pause). If organizers clarify REQ-055 allows re-open, flip to (b) — one-branch change in R6 + one test. Human decision gate.
