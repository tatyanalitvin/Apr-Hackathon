...
2. [ ] Flip `user.username` to `notNull()` in `packages/shared/src/schema.ts`, add `user.additionalFields.username = { type: "string", required: true, input: true }` to `auth.ts`, regenerate migration via `pnpm db:generate`, apply. Add zod-validated register/login body guards in front of the better-auth handler. Vitest unit test for each `registerSchema`/`loginSchema` failure mode (R5, R6).
