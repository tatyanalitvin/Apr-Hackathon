...
**Scope**: v4 REQ-099 (three-state presence), REQ-101 (server-side computation), REQ-102 (<2 s propagation), REQ-104 (cross-device union), REQ-105 (presence-under-ban). v4 REQ-100 (5 s `presence.activity` heartbeat) and REQ-103 (friends-and-room-co-members scoped visibility) are deferred / deviate — see `### ADR-0006 deviations` in §7. Extends the S1 binary presence (`REQ-041`) into the three-state model without breaking S1 subscribers.

**REQ-ID alignment vs. v4 catalog** (resolves earlier collision with `s2-friendship.md:24,61` — v4 REQ-105 = "Presence under ban" matches the S2 block-aware behavior):

| ID | v4 meaning | How S2 implements it |
| --- | --- | --- |
| REQ-099 | Three states: `online` ≥1 active tab, `afk` ≥1 connection no active tab 60 s, `offline` 0 connections | Implemented as the `PresenceState` union + emit-on-transition logic (R1, R2, R7) |
| REQ-100 | Client `presence.activity` heartbeat every ≤5 s while tab active; server derives AFK from `last_activity_at` | S2 uses client-pushed `presence.set("afk"/"online")` on the browser's 60 s idle timer — see ADR-0006 |
| REQ-101 | Server-side computation from `sessions` + connection registry; emits `presence.changed` | Implemented via Redis-backed per-user registry + snapshot key (R2, R10) |
| REQ-102 | 2 s P95 propagation under nominal load | Implemented as R9 latency test |
| REQ-103 | Visibility scoped to (a) friends and (b) room co-members; strangers don't see presence | S2 uses global `io.emit` fanout; scoping deferred — see ADR-0006 |
| REQ-104 | Union over devices/tabs — laptop active + phone background = `online` | Implemented as the multi-tab merge reducer (R5, R6) |
| REQ-105 | Banned user always appears `offline` to their banner; banner's state visible normally to the banned user | Implemented as block-aware filter on emit (R16) |

The REST endpoint `GET /api/v1/presence` supports REQ-101 (server-side snapshot computation) + R9's propagation SLO (the panel has to show a coherent state when it first renders). Not a separate REQ.
...
- As Alice (one tab, actively clicking), I appear as `online` to bob; my presence is emitted within the same tick my socket finishes the handshake. (v4 REQ-099, REQ-102)
- As Alice, when I don't interact with the tab for 60 seconds, my browser emits `presence.set("afk")`; the server fans out `presence.state` to bob with `state: "afk"` within 2s of the emit. Bob's member panel dot changes from green to yellow. (v4 REQ-099 afk state + REQ-102 latency; client-push deviates from REQ-100's heartbeat model — see ADR-0006)
- As Alice, when I move the mouse / type / click in the tab after being AFK, my browser emits `presence.set("online")`; the server fans out `state: "online"`. (deviates from v4 REQ-100 — see ADR-0006)
- As Alice with two tabs (Tab A active, Tab B idle for 5 minutes), I appear as `online` — the active tab wins. Bob never sees me as AFK. (v4 REQ-104 cross-device union)
- As Alice with two tabs both idle >1 min, I appear as `afk`. Both tabs have emitted `presence.set("afk")`; the merged state is `afk`. (v4 REQ-104)
- As Alice closing my last tab (or the browser crashing), my socket disconnects; my per-user presence becomes `offline`; the event fans out within 2s. (v4 REQ-099, REQ-102)
- As Alice with two tabs and I close one, I stay `online` (or `afk`, whatever the remaining tab says). Bob doesn't see a flap. (v4 REQ-104)
- As Bob subscribing to the room, on initial load I get the current presence state for each member via a REST endpoint (not an event replay) — see §5. After that, live updates arrive as `presence.state` events. The client ordering rule (§5 "REST-vs-socket race") ensures I never miss a transition that happened between the REST snapshot and my subscription. (v4 REQ-101 server-side computation)
- As Alice, when bob blocks me via `POST /api/v1/users/:id/block` (owned by `s2-friendship.md` R16), bob's sockets immediately see me as `offline` and my sockets see bob as `offline` — regardless of whether either of us is actually connected. Unblock restores the real state. (v4 REQ-105)
- As the demo operator, the member panel reflects alice's transitions to AFK and back consistent with the 1-min threshold, and reconnecting bob's browser doesn't show stale state. (v4 REQ-099, REQ-102)
...
- [ ] **R15 (transverse: socket auth still enforced)**: The presence.set handler receives the socket's bound userId from the handshake auth (S1 R9); no additional session check per event. Missing session on socket → connection already rejected. Test is transitive from the S1 better-auth socket-handshake coverage; no duplicate coverage.
...
S2 implements the presence intent of the v4 catalog but diverges from two REQs on the activity/visibility model. These deviations are deliberate and tracked in ADR-0006; none of them changes the demo behavior.

- **v4 REQ-100 — Activity signal from client (heartbeat model).** v4 wants a `presence.activity` event emitted by the client every ≤5 s while the tab is active, with the server deriving `afk` from `now() - last_activity_at > 60 s`. S2 ships **client-pushed transitions** instead: the browser owns its 60 s idle timer and emits `presence.set("afk")` / `presence.set("online")` on threshold crossing (see §5 "Client-side idle detection"). Rationale: the S1 protocol (`protocol.ts:83`) already defines `presence.set` with exactly this shape; adding a parallel heartbeat path would double the wire traffic and the server-side idle sweep is extra work the client can do for free. Functional result is identical from bob's perspective (alice's panel dot turns yellow within 2 s of idle threshold). Upgrade path: a future iteration can add `presence.activity` alongside `presence.set` without a protocol break — the reducer becomes a union of both sources.
- **v4 REQ-103 — Presence visibility scope (friends + room co-members).** v4 wants presence fanout restricted to recipients who either friend the subject or share a room with the subject; strangers see no state. S2 uses **global `io.emit`** fanout (see §4 R12 and §8 Q2 (a)). Rationale: at 300 concurrent users the all-to-all fanout is ~90k events per full transition round (sparse in practice) and the Redis adapter's pub/sub absorbs it cheaply; adding a per-transition membership lookup for scoped fanout is S3's job. Upgrade path: one-line flip from `io.emit` to `socket.rooms.forEach(r => io.to(r).emit(...))`-equivalent when S3 adds the scoping.

Both deviations are logged in FOLLOWUPS.md for the post-hackathon cleanup sweep. The block-aware overlay (R16, v4 REQ-105) still runs on top of whichever fanout scope is selected.
...
- [ ] `pnpm trace` covers REQ-099, REQ-101, REQ-102, REQ-104, REQ-105 (v4 REQ-100 heartbeat + REQ-103 scoped visibility deferred per §7 ADR-0006 deviations)
...
| --- | --- | --- |
| REQ-099 | Three-state coverage: first connect → `online`; `presence.set("afk")` after all tabs idle → `afk`; last disconnect → `offline`. Snapshot key drives emit-on-change. | integration (Socket.IO) |
| REQ-101 | Server computes merged state from the per-user Redis registry (`HGETALL` + R5 reducer); `GET /api/v1/presence` returns the same derivation for initial panel load. | integration (Redis + REST) |
| REQ-102 | p95 propagation latency < 2000 ms over 50 trials on ONLINE/AFK transitions; offline excluded because of the 3 s grace window. | integration (timing) |
| REQ-104 | Multi-tab union reducer: 1-online + 1-afk → `online`; 2-afk → `afk`; closing the only online tab flips the user to `afk` without emitting a spurious `offline`. | integration (Socket.IO) |
...
v4 REQ-100 (`presence.activity` 5 s heartbeat) and REQ-103 (scoped visibility to friends + room co-members) are not rows here — they are deferred with rationale in §7 "ADR-0006 deviations" and tracked in FOLLOWUPS.md.
