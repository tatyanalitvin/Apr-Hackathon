...
- [ ] **R8 — blocklist is loaded once, bounded size.** The `Set<string>` is module-scoped and populated at first import (not per `buildApp()`, not per request). Re-opening/re-reading on each sign-up would add a file syscall to the hot path. Note: compatible with the "one buildApp per Vitest file" memory — module-level IO fires once per worker process, orthogonal to better-auth init races. Set size is sanity-bounded (9500 ≤ size ≤ 10 000): Pwdb's 10k raw lines dedupe to 9789 lowercase entries (verified 2026-04-20), so the bound catches a truncated/wrong-file regression without breaking on upstream refreshes that shift the dedupe count.
