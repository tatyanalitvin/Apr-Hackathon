...
2. [ ] **Loader — TDD.** Write `apps/backend/tests/lib/password-blocklist.test.ts` FIRST covering: (a) empty-file throw (R7), (b) case-variant hit (`Password1234` matches), (c) random string miss, (d) loaded set has 10 000 entries. Commit red. Then implement `apps/backend/src/lib/password-blocklist.ts`: module-top-level `fs.readFileSync`, `Set<string>` of lowercased entries, exports `isCommonPassword(pw: string): boolean` that lowercases input before lookup. Commit green.
