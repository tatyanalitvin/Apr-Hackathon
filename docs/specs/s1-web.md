...
- **2026-04-18 (execution — fake-backend deferred)** — Plan Tasks 2–4 scaffolded a fake-backend path (`createFakeChatApi` + `createFakeChatSocket` behind `NEXT_PUBLIC_USE_FAKE_BACKEND`), but once the chat backend landed on `main` we rebased `feat/s1-web` onto it and flipped `USE_FAKE_BACKEND=0` for the rest of the slice. The fake path survives in tree as a demo aid (REQ-047 pill, REQ-048 older-page rehearsal without a running Fastify) but no page reaches for it. Decision: leave the fake modules in place, validate every page against the real backend as it lands, flag contract mismatches as commits instead of patching. Net effect on the §9 gate: the "fake mode" rows are descriptive-only; the "real mode" rows are the actual ship criterion.
- **2026-04-18 (execution — Socket.IO ack signature)** — Plan Task 13 drafted `socket.emit("room.subscribe", { roomId }, ack)`, but the typed protocol in `packages/shared/src/protocol.ts` is `(roomId: string, ack: (res: { ok; roomHeadSeq: string }) => void)` — positional string, not an object. Corrected before dispatch to the implementer subagent. Same for `room.unsubscribe`. Lesson for s2-web planning: cross-check Socket.IO event signatures against `protocol.ts` during plan writing, not execution.
- **2026-04-18 (execution — watermark hardening pass)** — Task 9's first pass landed the core state machine (ingest → backfill → emit) but code review flagged 2 Critical + 4 Important issues: stranded queue if `fetchHistory` throws, `reset()` didn't clear busy/queue, `primeFromAck` unconditionally overwrote `lastSeenSeq` (non-monotonic), empty-slice-despite-gap had no documented policy, backfill didn't filter out-of-window results, no `roomId` guard on ingest. Fix commit `55169ad` wrapped processEvent+drain in try/finally, added a generation counter so in-flight backfills bumped by `reset()` are detected and dropped, enforced monotonic `primeFromAck`, added an S1-scope comment on empty-slice policy (accept live event, advance), filtered backfill results by `< evt.seq`, and guarded ingest by roomId. 12/12 unit tests cover all branches including recover-after-reject and reset-mid-flight. Generation counter wasn't in the original spec but is the minimal way to implement "state clean after reset" when a `fetchHistory` is already awaiting — accepted as a judgment call.
- **2026-04-18 (execution — composer hardening pass)** — Task 10's first pass landed REQ-046 end-to-end but review flagged 3 Important issues: `onSend` rejection was unhandled (composer cleared its draft and silently ate the error), double-click could fire two `sendMessage` requests, and the aria-live byte counter blared to screen readers even when there was nothing to warn about. Fix commit `bfc7ca4` wrapped `await onSend(body)` in try/catch (preserve draft, return early on throw — parent owns error surfacing), added a `sendingRef` useRef re-entrance guard, and flipped `aria-hidden={bytes < SOFT_WARN_BYTES}` so SR only announces in the warning zone. Added a 9th composer test: preserves draft + re-enables Send when `onSend` rejects.
- **2026-04-18 (execution — vitest jsdom localStorage polyfill)** — jsdom 29 boots with inert `window.localStorage` (the `--localstorage-file` flag warning is emitted on start), so Task 10's `beforeEach(() => window.localStorage.clear())` would throw. Fix: `apps/web/vitest.setup.ts` does a feature-check (`typeof localStorage !== "undefined" && "setItem" in localStorage`) and, if storage is inert, installs a Map-backed Storage polyfill on `window.localStorage`. Feature-gated so the polyfill auto-disables the day jsdom fixes the underlying issue. Also needed `apps/web/tsconfig.json` to include `vitest.setup.ts` so `@testing-library/jest-dom/vitest` type augmentation resolves `toBeDisabled`/`toHaveValue` — staged 6 files for Task 10 instead of the plan's 4.
- **2026-04-18 (execution — Tailwind v3 / shadcn v3-default realignment)** — Plan assumed shadcn's `new-york-v4` style set and Tailwind v4, but the `feat/s1-web` branch inherits Tailwind 3.4.17 + tailwindcss-animate from `main`. Switched `components.json` to `style: "default"` + `baseColor: "slate"` and re-ran `shadcn add` for Button/Input/Label/Card/Avatar. Net result: primitives render identically, no class-name drift, and every downstream component in the plan works without rewrite. Noted here for s2-web: check `components.json` style before planning primitive installs.
- **2026-04-18 (execution — gate dry-run)** — At Task 14 time, automated gates green: `pnpm typecheck` ✅, `pnpm lint` ✅, `pnpm test:run` 29/29 ✅ (safe-next 5, utils 3, watermark 12, MessageComposer 9), `pnpm build` ✅ (7 static pages + `/rooms/[roomId]` dynamic). R12 grep (`Number\(.+seq\)`) returns 0 hits ✅. REQ-ID coverage: `rg "REQ-04[2-8]" apps/web/src` returns 7 hits across 6 files (REQ-042 register, REQ-043 login, REQ-044 rooms, REQ-045 room view, REQ-046 composer via test names, REQ-047/048 message list). Manual browser QA rows (§9 "real mode" — two-browser race, gap-fill under DevTools throttle, mobile accordion at 1023px, IME Enter, 3072-byte Send disable, sign-out, `?next=` round-trip, pill behavior, localStorage draft scoping) are the human QA pass post-merge; not gated by this commit.
