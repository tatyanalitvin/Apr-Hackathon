...
**Owner (agent)**: Claude Code — frontend agent (see `.human/FRONTEND_AGENT_BRIEF.md`)
...
- [ ] **R1 (REQ-042)**: `/register` renders an email + username + display-name + password form. Client-side zod validation via `registerSchema` from `@ai-herders/shared/dto` (same schema the Fastify preHandler enforces — one source of truth, no drift). On submit, calls `authClient.signUp.email({ email, password, name, username })` against `${BACKEND_URL}/api/auth/sign-up/email` with credentials. Success → `router.replace("/rooms")`. Duplicate email / duplicate username / weak password → surface the better-auth error message inline under the offending field via `<FormMessage />` and a `sonner` toast.
- [ ] **R2 (REQ-043)**: `/login` renders email + password + rememberMe checkbox. Zod validation via `loginSchema`. On submit, `authClient.signIn.email({ email, password, rememberMe })`. Success → `router.replace("/rooms")`. Wrong-creds → generic error surfaced as a toast + inline message; per s1-auth R9 the server returns an identical shape for wrong-email vs wrong-password, so the UI MUST NOT attempt to differentiate. Rate-limit (429 after 5 attempts/60s per s1-auth task #10) surfaces as "Too many attempts, wait a minute" — explicit string, not raw server error.
...
- [ ] **R5 (REQ-046)**: The composer is a `react-textarea-autosize` textarea growing 1-to-6 lines. Unit test (`vitest` + `@testing-library/react` + `user-event`) fences:
  ...
  - Body >3072 bytes (UTF-8) → Send button is `disabled` with a "too long" hint (same limit as `messageBodySchema`).
- [ ] **R6 (REQ-047)**: `MessageList` uses `react-virtuoso`'s `Virtuoso` component with `followOutput={(isAtBottom) => (isAtBottom ? "smooth" : false)}` + `atBottomStateChange={setIsAtBottom}` + `atBottomThreshold={50}`. Manual verification (two-browser, §9 checklist): user at bottom stays pinned when peer sends; user scrolled to mid-history remains at the same visible anchor when peer sends (no yank).
- [ ] **R7 (REQ-048)**: `MessageList` wires `Virtuoso`'s `startReached` callback to fetch the older page via `GET /api/v1/rooms/:id/messages?toSeq=${oldestLoadedSeq - 1n}&limit=50`, prepends the returned `messages` to local state, and updates `firstItemIndex` so Virtuoso preserves the anchor automatically. A loading sentinel (single-line skeleton) renders above the list during the fetch. If the returned page is empty, further `startReached` fires are ignored (set a `hasMore` flag).
...
`socket.io-client`, `better-auth`, `zod`, `clsx`, `tailwind-merge`, `lucide-react`, `@ai-herders/shared` already in `apps/web/package.json`. shadcn primitives (`button`, `input`, `label`, `card`, `form`, `scroll-area`, `separator`, `skeleton`, `avatar`, `sonner`, `textarea`) installed via `pnpm dlx shadcn@latest add …` as each task needs them.
...
1. [ ] **Worktree + deps + env** — `git worktree add` (done), `pnpm install`, add runtime deps (`react-hook-form`, `@hookform/resolvers`, `react-textarea-autosize`, `react-virtuoso`, `sonner`), create `.env.local.example`, verify `pnpm --filter web dev` boots against the existing shell page. Commit: `chore(s1-web): dependencies + env template`.
...
11. [ ] **`MessageList.tsx` (REQ-047, REQ-048, R6, R7)** — Virtuoso integration: `followOutput`, `atBottomStateChange`, `atBottomThreshold={50}`, `startReached` → prepend older page, `firstItemIndex` anchor preservation, `hasMore` gating. Item renderer is a simple bubble + author + timestamp. Manual verification: §9 rows 1, 2, 3. Commit: `feat(s1-web): REQ-047/048 message list (virtuoso)`.
...
- **shadcn `sheet` for mobile nav** — might be nicer than `<details>` below `lg:`; skipped for the smallest-slice rule. If a judge runs the demo at 768px and the accordion feels clumsy, S2 swaps it.
...
- [ ] `pnpm trace` counts REQ-042…REQ-048 as covered (page source + `watermark.test.ts` names + `MessageComposer.test.tsx` names).
